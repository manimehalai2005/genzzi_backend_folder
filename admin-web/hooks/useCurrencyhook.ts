...
export function useCurrency(id: string) {
  return useQuery<CurrencyResponse>({
    ...
    queryFn: () => currencyApi.getByCode(id),
    enabled: !!id, // Execute only if code exists
  });
}
...
export function useUpdateCurrency() {
  ...
  return useMutation({
    mutationFn: ({id , dto }: { id: string; dto: UpdateCurrencyDto }) =>
      currencyApi.update(id, dto),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['currencies'] });
      queryClient.invalidateQueries({ queryKey: ['currencies', variables.id] });
    },
  });
}
