...
import { LanguageTableProps } from '@/typess/language';
...
export function LanguageTable({ languages }: LanguageTableProps) {
  ...
  const { deleteLanguage, isDeleting } = useLanguageMutations();
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      await deleteLanguage(deleteTargetCode);
          ...
          <Table className="w-full text-left text-sm border-collapse">
            <TableHeader className="bg-muted/50 border-b border-border/40">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="p-4 sm:p-5 font-black text-lg uppercase tracking-wider text-muted-foreground">Native Name</TableHead>
            ...
            <TableBody className="divide-y divide-border/30">
              {languages.length === 0 ? (
                ...
                languages.map((lang) => (
                  <TableRow key={lang.code || lang.id} className="hover:bg-fuchsia-500/[0.03] transition-colors group">
                    <TableCell className="p-4 sm:p-5 font-mono text-xs font-semibold text-fuchsia-500 uppercase tracking-wide">
                      {lang.code}
                    </TableCell>
                    <TableCell className="p-4 sm:p-5 font-bold text-foreground text-lg">{lang.name}</TableCell>
                    <TableCell className="p-4 sm:p-5 font-medium text-muted-foreground">{lang.nativeName || '-'}</TableCell>
                    <TableCell className="p-4 sm:p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-black shadow-sm ${
                        ...
                        {(lang.status || 'ACTIVE') === 'ACTIVE' ? '🟢 Active' : '🟡 Inactive'}
                      </span>
                    </TableCell>
                    <TableCell className="p-4 sm:p-5 text-right space-x-2">
                      ...
                      <button
                        onClick={() => confirmDelete(lang.code, lang.name)}
