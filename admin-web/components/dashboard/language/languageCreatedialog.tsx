...
export function LanguageCreateDialog() {
  ...
  const { createLanguage, isCreating } = useLanguageMutations();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    try {
      await createLanguage({ code, name, nativeName });
      setCode('');
      setName('');
      setNativeName('');
      setOpen(false);
...
>
  <Plus className="w-4 h-4 stroke-[3]" /> Add Language ✨
</DialogTrigger>
      ...
      >
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Setup system localization codes, names, and native strings.
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
          <div className="space-y-1.5">
            ...
            >
              Language Code (e.g., ta, en) <span className="text-fuchsia-500">*</span>
            </Label>
            <Input
              id="code"
              value={code}
