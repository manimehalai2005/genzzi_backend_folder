...
import { IndustryEditDialog } from './IndustriesEditDialog';
...
import { IndustryTableProps } from '@/interface/Industries/InsdustryTableProps';
import { useDeleteIndustry } from '@/hooks/useIndustrieshook';
...
export function IndustryTable({
  ...
  const data = Array.isArray(rawData)
    ...
    : Array.isArray((rawData as any)?.data)
      ? (rawData as any).data
      : Array.isArray((rawData as any)?.industries)
        ? (rawData as any).industries
        : [];
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      await deleteIndustry(deleteTargetId);
      ...
      toast.success('Industry deleted successfully! 🗑️');
    } catch (err: any) {
      const errorMessage = err?.message || 'Failed to delete industry';
                ...
                <TableRow className="hover:bg-transparent">
                  ...
                  >
                    <div className="inline-flex items-center gap-2">
                      ...
                      No industries found in the matrix.
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                data.map((industry: Industry) => {
                  const isActive = (industry.status || 'ACTIVE') === 'ACTIVE';
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Description */}
                      <TableCell className="p-4 sm:p-5 font-medium text-muted-foreground max-w-[400px]">
                      ...
                      <TableCell className="p-4 sm:p-5 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              confirmDelete(industry.id, industry.name)
                            }
            ...
            return (
              ...
              >
                ...
                <div className="rounded-2xl border border-border/40 bg-muted/20 p-3">
                  ...
                  <p className="text-sm font-medium text-foreground mt-1 leading-relaxed break-words">
                    {industry.description || '—'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  ...
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={() => confirmDelete(industry.id, industry.name)}
