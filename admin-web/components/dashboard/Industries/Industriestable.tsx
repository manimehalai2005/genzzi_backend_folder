...
import { useIndustryMutations } from '@/hooks/useIndustrieshook';
...
import { IndustryTableProps } from '@/typess/Industries';
...
export function IndustryTable({ industries }: IndustryTableProps) {
  ...
  const { deleteIndustry, isDeleting } = useIndustryMutations();
            ...
            <TableBody className="divide-y divide-border/30">
              {industries.length === 0 ? (
                ...
                industries.map((industry) => (
                  <TableRow key={industry.id} className="hover:bg-fuchsia-500/[0.03] transition-colors group">
                    <TableCell className="p-4 sm:p-5 font-bold text-foreground text-lg">{industry.name}</TableCell>
                    ...
                    <TableCell className="p-4 sm:p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-black shadow-sm ${
                        (industry.status || 'ACTIVE') === 'ACTIVE' 
                          ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' 
                          : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                      }`}>
                        {(industry.status || 'ACTIVE') === 'ACTIVE' ? '🟢 Active' : '🟡 Inactive'}
                      </span>
                    </TableCell>
