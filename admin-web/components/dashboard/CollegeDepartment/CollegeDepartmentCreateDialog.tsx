...
import { useCreateCollegeDepartment } from '@/hooks/useCollegeDepartment';
import { CreateCollegeDepartmentDto } from '@/dto/CollegeDepartment';
...
export function CollegeDepartmentCreateDialog() {
  ...
  const { mutateAsync: createCollegeDepartment, isPending: isCreating } = useCreateCollegeDepartment();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: CreateCollegeDepartmentDto = {
        collegeId,
        departmentId,
      };

      await createCollegeDepartment(payload);
      
      setCollegeId('');
      setDepartmentId('');
      setOpen(false);
      toast.success('College department created successfully! ✨');
    } catch (err: any) {
      console.error('Create Error:', err);
      const errorMessage = err?.response?.data?.message || err?.message || 'Failed to create college department';
