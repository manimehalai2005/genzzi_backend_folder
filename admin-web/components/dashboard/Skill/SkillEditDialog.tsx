...
import { SkillEditDialogProps } from '@/typess/Skill';
...
export function SkillEditDialog({ skill, open, onOpenChange }: SkillEditDialogProps) {
  ...
  const { updateSkill, isUpdating } = useSkillMutations();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateSkill({
        id: skill.id,
        dto: { name, category, description, status },
      });
      onOpenChange(false);
    } catch (err: any) {
      ...
      >
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Update technical skill attributes, categories, descriptions, and statuses.
          </p>
        </DialogHeader>
