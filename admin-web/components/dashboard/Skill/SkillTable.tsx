...
export function SkillTable({
  ...
  const data = Array.isArray(rawData)
    ...
    : Array.isArray((rawData as any)?.data)
      ? (rawData as any).data
      : Array.isArray((rawData as any)?.skills)
        ? (rawData as any).skills
        : [];
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      const errorMessage = err?.message || 'Failed to delete skill';
                      ...
                      <TableCell className="p-4 sm:p-5 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              confirmDelete(skill.id ?? '', skill.name ?? '')
                            }
      ...
      <div className="md:hidden space-y-4">
        {isLoading ? (
          ...
          <div className="border border-dashed border-fuchsia-500/30 rounded-3xl bg-card/95 shadow-xl p-10 text-center">
            ...
            <p className="text-sm font-bold text-foreground">No skills found</p>
