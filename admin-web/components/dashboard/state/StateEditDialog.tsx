...
import { Sparkles, AlertCircle, Check, ChevronsUpDown } from 'lucide-react';
import { StateEditDialogProps } from '@/typess/State';
...
export function StateEditDialog({ state, isOpen, onClose }: StateEditDialogProps) {
  ...
  const { updateState, isUpdating } = useStateMutations();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(error?.message || 'Failed to update state. Please try again.');
    }
  };
