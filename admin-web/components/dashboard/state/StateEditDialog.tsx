...
export function StateEditDialog({ state, isOpen, onClose }: StateEditDialogProps) {
  ...
  // Safely normalize countriesData to guarantee an array (fixes `find` / `map` errors)
  const countriesList = Array.isArray(countriesData)
    ? countriesData
    : Array.isArray((countriesData as any)?.data)
    ? (countriesData as any).data
    : Array.isArray((countriesData as any)?.countries)
    ? (countriesData as any).countries
    : [];

  const selectedCountry = countriesList.find((c: any) => String(c.id) === String(formData.countryId));
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5 flex flex-col">
            ...
            <Popover open={comboboxOpen} onOpenChange={setComboboxOpen}>
              {/* Fix: Added asChild here to prevent nested button validation warnings */}
              <PopoverTrigger>
                <Button
                  variant="outline"
                  role="combobox"
