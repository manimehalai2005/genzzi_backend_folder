...
export function StateCreateDialog() {
  ...
  const { createState, isCreating } = useStateMutations();
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5 flex flex-col">
            ...
            <Popover open={comboboxOpen} onOpenChange={setComboboxOpen}>
              ...
              <PopoverContent className=" w-full p-0 rounded-2xl bg-card border-border/40 shadow-2xl backdrop-blur-2xl">
                <Command>
                    ...
                    <CommandGroup>
                      {countriesList.map((country) => (
                        <CommandItem
                          ...
                          className=" w-full flex items-center justify-between text-sm font-medium rounded-xl px-3 py-2 cursor-pointer hover:bg-fuchsia-500/10 aria-selected:bg-fuchsia-500/15"
                        >
                          {country.name}
                          <Check
                            className={cn(
