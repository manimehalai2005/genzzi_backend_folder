...
import { CollegeEditDialogProps } from '@/typess/College';
import {
  Select,
  SelectContent,
  SelectItem,
...
export function CollegeEditDialog({ college, open, onOpenChange }: CollegeEditDialogProps) {
  const [name, setName] = useState('');
  const [code, setCode] = useState('');
  const [website, setWebsite] = useState('');
  const [status, setStatus] = useState('ACTIVE');

  const { updateCollege, isUpdating } = useCollegeMutations();
