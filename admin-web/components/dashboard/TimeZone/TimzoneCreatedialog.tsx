...
export function TimezoneCreateDialog() {
  ...
  const { createTimezone, isCreating } = useTimezoneMutations();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    try {
      await createTimezone({ name, offset, gmtOffset, abbreviation });
      setName('');
      setOffset('');
      setGmtOffset('');
      setAbbreviation('');
      ...
      setError(err.message || 'Failed to create timezone');
    }
  };
