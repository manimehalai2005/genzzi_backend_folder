...
import { TimezoneEditDialogProps } from '@/typess/TimeZone';
...
export function TimezoneEditDialog({ timezone, open, onOpenChange }: TimezoneEditDialogProps) {
  ...
  const { updateTimezone, isUpdating } = useTimezoneMutations();

  useEffect(() => {
    if (timezone) {
      setName(timezone.name || '');
      setOffset(timezone.offset || '');
      setGmtOffset(timezone.gmtOffset || '');
      setAbbreviation(timezone.abbreviation || '');
      setStatus(timezone.status || 'ACTIVE');
      setError('');
    }
  }, [timezone]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateTimezone({
        id: timezone.id,
        dto: { name, offset, gmtOffset, abbreviation, status },
      });
      onOpenChange(false);
    } catch (err: any) {
      ...
      >
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Modify global offset configurations, status flags, and abbreviations.
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="edit-offset" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
              Offset
            </Label>
            <Input
              id="edit-offset"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="edit-gmtOffset" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
              GMT Offset
            </Label>
            <Input
              id="edit-gmtOffset"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="edit-abbreviation" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
              Abbreviation
            </Label>
            <Input
          ...
          <div className="space-y-1.5">
            <Label htmlFor="edit-status" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
              Status
            </Label>
            <Input
              id="edit-status"
