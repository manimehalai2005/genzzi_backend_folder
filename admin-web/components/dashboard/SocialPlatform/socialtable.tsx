...
import { SocialPlatformEditDialog } from './SocialEditdialog';
...
import { SocialPlatformTableProps } from '@/typess/Socialplatform';

export function SocialPlatformTable({ platforms }: SocialPlatformTableProps) {
  ...
  const { deleteSocialPlatform, isDeleting } = useSocialPlatformMutations();
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      await deleteSocialPlatform(deleteTargetId);
  ...
  return (
    <div className="space-y-4">
          ...
          <Table className="w-full text-left text-sm border-collapse">
            <TableHeader className="bg-muted/50 border-b border-border/40">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="p-4 sm:p-5 font-black text-lg uppercase tracking-wider text-muted-foreground">Code</TableHead>
                <TableHead className="p-4 sm:p-5 font-black text-lg uppercase tracking-wider text-muted-foreground">Base URL</TableHead>
                <TableHead className="p-4 sm:p-5 font-black text-lg uppercase tracking-wider text-muted-foreground">Icon</TableHead>
                <TableHead className="p-4 sm:p-5 font-black text-lg uppercase tracking-wider text-muted-foreground">Status</TableHead>
                <TableHead className="p-4 sm:p-5 text-righ  font-black text-lg uppercase tracking-wider text-muted-foreground">Actions</TableHead>
              </TableRow>
            </TableHeader>
                ...
                <TableRow className="hover:bg-transparent">
                  <TableCell colSpan={6} className="p-12 text-center text-muted-foreground font-medium">
                    <div className="inline-flex items-center gap-2">
                      ...
                      No social platforms found in the matrix.
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                platforms.map((platform) => (
                  <TableRow key={platform.id} className="hover:bg-fuchsia-500/[0.03] transition-colors group">
                    <TableCell className="p-4 sm:p-5 font-bold text-foreground text-lg">{platform.name}</TableCell>
                    <TableCell className="p-4 sm:p-5 font-mono text-xs font-semibold text-fuchsia-500 uppercase tracking-wide">
                      {platform.code || '-'}
                    </TableCell>
                    <TableCell className="p-4 sm:p-5 font-medium text-muted-foreground truncate max-w-[200px]">
                      {platform.baseUrl || '-'}
                    </TableCell>
                    <TableCell className="p-4 sm:p-5 font-medium text-muted-foreground">{platform.icon || '-'}</TableCell>
                    <TableCell className="p-4 sm:p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-black shadow-sm ${
                        ...
                        {(platform.status || 'ACTIVE') === 'ACTIVE' ? '🟢 Active' : '🟡 Inactive'}
                      </span>
                    </TableCell>
      ...
      {/* Edit Social Platform Dialog */}
      <SocialPlatformEditDialog
