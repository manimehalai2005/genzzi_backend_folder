...
import { useDeleteSocialPlatform } from '@/hooks/useSocialPlatform';
import { SocialPlatform } from '@/typess/Socialplatform';
import { SocialPlatformTableProps } from '@/interface/Socialplatform/SocialTableProps';
import { SocialPlatformEditDialog } from './SocialEditdialog';

export function SocialPlatformTable({
  data: rawData,
  isLoading,
  onEdit,
  onDelete,
}: SocialPlatformTableProps) {
  // Safely normalize rawData to always guarantee an array
  const platforms = Array.isArray(rawData)
    ? rawData
    : Array.isArray((rawData as any)?.data)
      ? (rawData as any).data
      : Array.isArray((rawData as any)?.platforms)
        ? (rawData as any).platforms
        : [];
  ...
  const { mutateAsync: deleteSocialPlatform, isPending: isDeleting } =
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      toast.success('Social platform deleted successfully! 🗑️');
    } catch (err: any) {
      const errorMessage = err?.message || 'Failed to delete social platform';
                ...
                platforms.map((platform: SocialPlatform) => (
                  ...
                  >
                    ...
                    <TableCell className="p-4 sm:p-5">
                      <span className="font-mono text-xs font-semibold text-fuchsia-500 tracking-wide break-all">
                        {platform.website || '-'}
                      </span>
                    </TableCell>
        ...
        {isLoading ? (
          <div className="border border-border/40 rounded-3xl bg-card/95 shadow-xl p-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Loading social platform matrix...
            </p>
          </div>
        ) : platforms.length === 0 ? (
          /* =========================
          ...
          <div className="border border-dashed border-fuchsia-500/30 rounded-3xl bg-card/95 shadow-xl p-10 text-center">
            <Share2 className="w-8 h-8 mx-auto mb-3 text-fuchsia-500" />
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="rounded-2xl border border-border/40 bg-muted/20 p-3">
                  ...
                  <p className="text-sm font-mono font-black text-fuchsia-400 mt-1 break-all">
                    {platform.website || '-'}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={() =>
                    confirmDelete(platform.id ?? '', platform.name ?? '')
                  }
