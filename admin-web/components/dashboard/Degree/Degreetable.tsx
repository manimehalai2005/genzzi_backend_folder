...
import { DegreeTableProps } from '@/typess/degree';


export function DegreeTable({ degrees }: DegreeTableProps) {
  ...
  const { deleteDegree, isDeleting } = useDegreeMutations();
          ...
          <Table className="w-full text-left text-sm border-collapse">
            <TableHeader className="bg-muted/50 border-b border-border/40">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="p-4 sm:p-5 font-black text-lg uppercase tracking-wider text-muted-foreground">Level</TableHead>
            ...
            <TableBody className="divide-y divide-border/30">
              {degrees.length === 0 ? (
                ...
                degrees.map((degree) => (
                  <TableRow key={degree.id} className="hover:bg-fuchsia-500/[0.03] transition-colors group">
                    <TableCell className="p-4 sm:p-5 font-bold text-foreground text-lg">{degree.name}</TableCell>
                    ...
                    <TableCell className="p-4 sm:p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-black shadow-sm ${
                        ...
                        {(degree.status || 'ACTIVE') === 'ACTIVE' ? '🟢 Active' : '🟡 Inactive'}
                      </span>
                    </TableCell>
                    <TableCell className="p-4 sm:p-5 text-right  text-sm space-x-2">
                      <button
