...
import { useUpdateDegree } from '@/hooks/useDegreehook';
import { DegreeEditDialogProps } from '@/interface/Degree/DegreeEditProps';

export function DegreeEditDialog({ degree, open, onOpenChange }: DegreeEditDialogProps) {
  ...
  useEffect(() => {
    if (degree) {
      ...
      setStatus(degree.status || 'ACTIVE');
    }
  }, [degree]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateDegree({
        id: degree.id,
        dto: { name, code, level, status },
      });
      onOpenChange(false);
      toast.success('Degree updated successfully! 💾');
    } catch (err: any) {
      const errorMessage = err?.message || 'Failed to update degree';
      ...
      >
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          <DialogTitle className="text-xl sm:text-2xl font-black tracking-tight bg-gradient-to-r from-foreground via-fuchsia-400 to-brand-orange bg-clip-text text-transparent">
            Edit Degree: {degree?.code || degree?.name} 
          </DialogTitle>
          <p className="text-xs text-muted-foreground font-medium">
            Update program parameters, degree level, and active status configurations.
          </p>
        </DialogHeader>
