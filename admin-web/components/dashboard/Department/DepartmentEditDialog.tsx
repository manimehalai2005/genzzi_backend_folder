...
import { DepartmentEditDialogProps } from '@/typess/Department';



export function DepartmentEditDialog({ department, open, onOpenChange }: DepartmentEditDialogProps) {
  ...
  const { updateDepartment, isUpdating } = useDepartmentMutations();
      ...
      >
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          ...
          <DialogTitle className="text-xl sm:text-2xl font-black tracking-tight bg-gradient-to-r from-foreground via-fuchsia-400 to-brand-orange bg-clip-text text-transparent">
            Edit Department: {department?.code || department?.name}
          </DialogTitle>
          <p className="text-xs text-muted-foreground font-medium">
            Update department metadata, structural codes, and active status configurations.
          </p>
        </DialogHeader>
