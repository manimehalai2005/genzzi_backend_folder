...
import { useDepartmentMutations } from '@/hooks/useDepartmenthook';
...
import { DepartmentTableProps } from '@/typess/Department';
            ...
            <TableBody className="divide-y divide-border/30">
              {departments.length === 0 ? (
                ...
                departments.map((department) => (
                  <TableRow key={department.id} className="hover:bg-fuchsia-500/[0.03] transition-colors group">
                    <TableCell className="p-4 sm:p-5 font-bold text-foreground text-lg">{department.name}</TableCell>
                    ...
                    <TableCell className="p-4 sm:p-5">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-black shadow-sm ${
                        ...
                        {(department.status || 'ACTIVE') === 'ACTIVE' ? '🟢 Active' : '🟡 Inactive'}
                      </span>
                    </TableCell>
