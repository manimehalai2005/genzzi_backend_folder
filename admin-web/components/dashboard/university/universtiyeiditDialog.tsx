...
import { UniversityEditDialogProps } from '@/typess/University';
...
export function UniversityEditDialog({ university, open, onOpenChange }: UniversityEditDialogProps) {
  ...
  const { updateUniversity, isUpdating } = useUniversityMutations();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateUniversity({
        id: university.id,
        dto: { name, code, countryId, website, status },
      });
      onOpenChange(false);
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to update university');
    }
  };
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="edit-code" className="text-xs font-black uppercase tracking-wider text-muted-foreground">Code</Label>
              <Input
                id="edit-code"
                value={code}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="edit-status" className="text-xs font-black uppercase tracking-wider text-muted-foreground">Status</Label>
            <Input
              id="edit-status"
              value={status}
