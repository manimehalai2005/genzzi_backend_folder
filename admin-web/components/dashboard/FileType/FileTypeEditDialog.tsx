...
export function FileTypeEditDialog({ fileType, open, onOpenChange }: FileTypeEditDialogProps) {
  ...
  const { updateFileType, isUpdating } = useFileTypeMutations();
  ...
  useEffect(() => {
    if (fileType) {
      ...
      setMaxSizeInMb(fileType.maxSizeInMb ? fileType.maxSizeInMb.toString() : '');
      setStatus(fileType.status || 'ACTIVE');
    }
  }, [fileType]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update file type');
    }
  };
      ...
      >
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          ...
          <DialogTitle className="text-xl sm:text-2xl font-black tracking-tight bg-gradient-to-r from-foreground via-fuchsia-400 to-brand-orange bg-clip-text text-transparent">
            Edit File Type: {fileType?.extension || fileType?.name} 
          </DialogTitle>
          <p className="text-xs text-muted-foreground font-medium">
            Update file specifications, MIME configurations, and size limits.
          </p>
        </DialogHeader>
