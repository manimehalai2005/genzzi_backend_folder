...
import { useCreateFileType } from '@/hooks/useFileType';
...
export function FileTypeCreateDialog() {
  ...
  const { mutateAsync: createFileType, isPending: isCreating } = useCreateFileType();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err?.message || 'Failed to create file type';
  ...
  Add Filtype
      ...
      <DialogContent className="fixed left-1/2 top-1/2 z-50 grid w-[calc(100vw-2rem)] max-w-[480px] -translate-x-1/2 -translate-y-1/2 gap-5 rounded-3xl bg-card/95 border-border/40 p-6 sm:p-7 shadow-2xl backdrop-blur-2xl duration-200 max-h-[90vh] overflow-y-auto modal-scrollbar">
        ...
        <DialogHeader className="space-y-1.5 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Setup system file specifications, extensions, and mime types.
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="mimeType" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
