...
import { useDeleteFileType } from '@/hooks/useFileType';
import { FileTypeTableProps } from '@/interface/FileType/FiteTypeTableProps';
...
export function FileTypeTable({
  ...
  const data = Array.isArray(rawData)
    ...
    : Array.isArray((rawData as any)?.data)
      ? (rawData as any).data
      : Array.isArray((rawData as any)?.fileTypes)
        ? (rawData as any).fileTypes
        : [];
  ...
  const { mutateAsync: deleteFileType, isPending: isDeleting } =
    useDeleteFileType();
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      toast.success('File type deleted successfully! 🗑️');
    } catch (err: any) {
      const errorMessage = err?.message || 'Failed to delete file type';
              ...
              {isLoading ? (
                ...
                data.map((fileType: FileType) => (
                  ...
                  >
                    ...
                    {/* Mime Type */}
                    <TableCell className="p-4 sm:p-5 font-mono text-sm font-semibold text-fuchsia-500 break-all">
                      {fileType.mimeType}
                    </TableCell>
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-fuchsia-500/10 border border-fuchsia-500/20 flex items-center justify-center">
                  <FileIcon className="w-6 h-6 text-fuchsia-400" />
                </div>
                ...
                <div className="min-w-0">
                  ...
                  <h3 className="text-base font-black text-foreground break-all">
                    {fileType.extension || '—'}
                  </h3>
                </div>
              </div>
