...
export default function CertificationProviderPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:space-y-8 sm:p-8">
      <div className="relative flex flex-col items-start justify-between gap-4 overflow-hidden rounded-3xl border border-fuchsia-500/20 bg-gradient-to-r from-violet-500/10 via-fuchsia-500/10 to-brand-orange/10 p-6 shadow-2xl backdrop-blur-2xl sm:flex-row sm:items-center sm:p-8">
        ...
        <div className="relative z-10 space-y-1.5">
          ...
          <p className="max-w-md text-xs font-medium text-muted-foreground sm:text-sm">
            Manage certification providers, affiliations, websites, codes, and status in one streamlined flow.
          </p>
        </div>
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-fuchsia-500/20 bg-card/40 py-24 shadow-xl backdrop-blur-xl">
          ...
          <p className="text-sm font-black bg-gradient-to-r from-fuchsia-500 to-brand-orange bg-clip-text text-transparent">
      ...
      {!isLoading && !error && (
        <CertificationProviderTable providers={providers} />
      )}
    </div>
  );
}